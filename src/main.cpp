...
PyNodeData::PyNodeData(v8::Isolate *isolate, v8::Local<v8::Object> exports)
    : pModule(NULL) {
  exports_.Reset(isolate, exports);
  exports_.SetWeak(this, DeleteMe, v8::WeakCallbackType::kParameter);
}

void PyNodeData::DeleteMe(const v8::WeakCallbackInfo<PyNodeData> &info) {
...
//   public:
CallWorker::CallWorker(Nan::Callback *callback, PyObject *pyArgs,
                       PyObject *pFunc)
    : Nan::AsyncWorker(callback), pyArgs(pyArgs), pFunc(pFunc){};
CallWorker::~CallWorker() {}

void CallWorker::Execute() {}

void CallWorker::HandleErrorCallback() {
...
void CallWorker::HandleOKCallback() {
  Nan::HandleScope scope;
...

  {
    ...
    if (pValue != NULL) {
      ...
      if (strcmp(pValue->ob_type->tp_name, "NoneType") == 0) {
        ...
        // come before PyLong_Check (ie, PyBool_Type is always a PyLong_Type
        // but, a PyLong_Type is not necessarily a PyBool_Type)
      ...
      if (errOccurred != NULL) {
        ...
        while (frame != NULL) {
          ...
          stream << "File \"" << filename << "\", line " << line << ", in "
                 << funcname << "\n";
          stream << type << ": " << value;
          frame = frame->f_back;
        }
...
void call(const v8::FunctionCallbackInfo<v8::Value> &info) {
    ...
    if (pFunc != NULL && callable == 1) {
      ...
      if (passedArgsCount != pythonArgsCount) {
        ...
        size_t len = (size_t)snprintf(
            NULL, 0, "The function '%s' has %d arguments, %d were passed",
            *functionName, pythonArgsCount, passedArgsCount);
        error = (char *)malloc(len + 1);
        snprintf(error, len + 1,
                 "The function '%s' has %d arguments, %d were passed",
                 *functionName, pythonArgsCount, passedArgsCount);
  ...
  Nan::AsyncQueueWorker(new CallWorker(
      new Nan::Callback(
          Nan::To<v8::Function>(info[info.Length() - 1]).ToLocalChecked()),
      pArgs, pFunc));
}
...
NODE_MODULE_INITIALIZER(v8::Local<v8::Object> exports,
  ...
  exports
      ->Set(context,
            v8::String::NewFromUtf8(isolate, "startInterpreter",
  ...
  // exports->Set(context,
  //              v8::String::NewFromUtf8(isolate, "stopInterpreter",
  //              v8::NewStringType::kNormal)
  //                 .ToLocalChecked(),
  ...
  exports
      ->Set(context,
            v8::String::NewFromUtf8(isolate, "openFile",
                                    v8::NewStringType::kNormal)
                .ToLocalChecked(),
